<template>
  <div class="new-search-table-pagination">
        ...
        >
         <div class="page-content">
            ...
            <el-dropdown trigger="click">
                <div>{{pagination.pageIndex}}/{{pageTotal}} 页 <i class="el-icon-caret-bottom el-icon--right" style="color:#A2ACBA"/></div>
                <el-dropdown-menu slot="dropdown" class="pageination-dialog">
                  <div class="page-sizes-wrapper">
                      <div>每页展示</div>
                      <div class="page-sizes-group">
                            <div
                                class="sizes-item"
                                :class="{'sizes-item--current': +pagination.pageSize === +pageSize }"
                                v-for="pageSize in pageSizes"
                                @click="handleSizeChange(pageSize)"
                                :key="pageSize">
                                {{pageSize}}
                            </div>
                      </div>
                  ...
                  <div class="page-index-wrapper">
                        <div
                            ...
                            :class="{'page-index-item--current':+pagination.pageIndex===+item }"
                            @click="handleCurrentChange(item)"
                            v-for="item in pageIndexArray"
                            :key="item">
                          {{item}}
                        </div>
                  </div>
                  <div class="page-go-wrapper">
                      <div>前往页码</div>
                      <div class="go-input-wrapper">
                          <m-input
                            ...
                            decimal="0"
                            placeholder="" />
                          <div class="go-button" :class="{'go-button--disabled': !goPageIndex}" @click="handleCurrentChange(+goPageIndex)">
            ...
            <div class="pre-and-next-wrapper">
                <div class="pre-wrapper" :class="{'move-icon--disabled':+pagination.pageIndex===1}" @click="handleCurrentChange(pagination.pageIndex - 1)"><i class="el-icon-arrow-left" /></div>
                <div class="next-wrapper" :class="{'move-icon--disabled':+pagination.pageIndex===pageTotal}"  @click="handleCurrentChange(pagination.pageIndex + 1)"><i class="el-icon-arrow-right" /></div>
            </div>
         </div>
        </el-pagination>
        <div class="pagination-left-wrapper" v-show="isSelectable">
            <div>已选 {{selectionList.length}} 项</div>
        </div>
    </div>
    <el-dialog
      title="自定义显示列"
...
export default {
  name: "NewElSearchTablePagination",
  components: {
    searchForm,
    Draggable,
    EmptyContainer,
  ...
  computed: {
    ...
    pageIndexArray() {
      let firstIndex = this.pagination.pageIndex;
      const result = [];
      const resultLength = +this.pageTotal < 5 ? +this.pageTotal : 5;
      const diff = this.pageTotal - this.pagination.pageIndex;
      for (let i = 0; i < resultLength; i++) {
        if (diff < 5) {
          result.push(this.pageTotal + i - diff);
        } else {
    ...
    isSelectable() {
        return this.columnsNow.findIndex(item=>item.type==='selection') > -1;
    }
  },
  watch: {
    data(value) {
  ...
  methods: {
    ...
    resizeHeight() {
      this.$nextTick(() => {
        ...
        const elSearchTableHeight = this.$el.clientHeight;
         console.log('elSearchTableHeight',elSearchTableHeight)
        const tableSearchHeight = this.$el.querySelector('.table-search-wrap').clientHeight;
        const tableFormContainerHeight = this.$el.querySelector('.table-form-container').clientHeight + 8;
        const height = elSearchTableHeight - tableSearchHeight - tableFormContainerHeight - 44;
        console.log(height)
   ...
   // 如果有使用slot，请使用传入fetch方法获取导出参数
    ...
    handleCurrentChange(pageIndex) {
      ...
      this.pagination.pageIndex = pageIndex > this.pageTotal? this.pageTotal : pageIndex;
      this.dataChangeHandler();
    },
          ...
          if (Object.prototype.toString.call(response) === "[object Array]") {
            ...
            if (totalField && totalField.indexOf(".") !== -1) {
              ...
              console.log(totalValue,'------')
                this.pageTotal = response[pageTotalField];
            }
          } else {
            totalValue = 0;
          }
